...
| `insta --agent template list/info/deploy` [`--region <r>`] | `insta_template_search` / `insta_template_get` / `insta_template_deploy` (`region` parameter, slugs from `insta_regions`) / `insta_template_deployment_status` |
