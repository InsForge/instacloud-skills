...
| `cron.write`, `cron.run` | allow | creating, editing, pausing and firing a schedule (`insta cron create`/`edit`/`pause`/`resume`/`run`). A schedule reaches no further than deployed code — HTTPS GET/POST to a public address — and moving a job that is a POST or sends headers to a new target requires resending its request, so its stored header values and body never follow it somewhere new |
| `cron.delete` | **approve** | `insta cron delete` — cannot be undone: there is no undelete, and the job's run history stops being readable |
