...
| `services: [{type: web, name: X}]` | `insta --agent services add compute X --port <n>`. `--port` is optional and stores **`null`**, not `8080`; the 8080 default is applied at *deploy* time. Pass it anyway, and pass it again on `connect-repo` (the port-overwrite note in `../migrate.md`) |
...
| `type: pserv` (private service) | a compute service, but **flag it to the user**: every compute service gets a public default domain with its first successful deploy (step 1 of the cutover in `../migrate.md`; `services add` alone leaves `domain: null`), so a Render private service stops being unreachable from the internet |
