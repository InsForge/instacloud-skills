...
same-name collision, and `insta --agent feedback` (CLI ≥ 0.1.4) to refuse a signed-out or staging
...
**(CLI ≥ 0.1.4)** Signed in to InstaCloud (a session or an `insta_` key), the report opens a support
...
**refuses with exit 2** (`{"status":"refused"}` under `--json`): run `insta login`, then send it
