...
| `insta --agent cron list` [`--branch <b>`] [`--json`] · `insta --agent cron show <name>` [`--json`] | **(CLI ≥ 0.1.8)** the branch's schedules / one of them. `show` lists header **names** only — values are encrypted at rest and the API never returns them, so an empty value column would read as "unset" rather than "not shown". A secret-backed header shows the secret it reads instead: `x-api-key ← secret STRIPE_KEY` (gated: `cron.read`) |
...
run is `failed`); `secret_resolver_error` is transient and retried. Keep `--header` for non-secret values.
