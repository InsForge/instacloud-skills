...
| `insta --agent build-logs <build-id>` [`--source <archive\|github>`] [`--follow`] [`--json`] | source-build steps/output via the Platform API. Default source is `archive`: copy the concrete `insta build-logs <id>` command printed when `insta deploy` accepts the operation (stderr with `deploy --json`). For `--source github`, use `build.buildId` from `insta compute connect-repo <owner/repo> --json`, or `builds[].id` from `GET /projects/{id}/github/builds`. `--follow` polls while building and briefly after completion for delayed output. `--json` prints one snapshot and cannot be combined with `--follow`. Governed by `logs.read`. Source-directory deploys also print new build output while waiting; with `deploy --json`, progress/logs go to stderr and stdout remains the result JSON. No Depot credentials are needed locally |
