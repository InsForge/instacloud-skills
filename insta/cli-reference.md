...
| `insta --agent compute scale [service] --remove <instance>` [`--branch <b>`] [`--json`] | (CLI ≥ 0.1.8 — older builds reject `--remove`; check with `insta --agent compute scale --help`) drop **one named instance** and run one fewer replica — the scale-in to use when you know which replica should go (e.g. the one whose long-lived connections have ended). The instance id (`inst-<12 hex>`) is the `instance` field of `insta --agent compute logs --json`. Same gates as `scale` (paid plans; gated: `service.scale`). Refused for the **last** instance (400), for a service that is not running (409), and while another scale of the same service is in flight (409 — retry once it finishes). Takes no count and no `--region` |
...
| `insta --agent cron list` [`--branch <b>`] [`--json`] · `insta --agent cron show <name>` [`--json`] | **(CLI ≥ 0.1.3)** the branch's schedules / one of them. `show` lists header **names** only — values are encrypted at rest and the API never returns them, so an empty value column would read as "unset" rather than "not shown". **(the CLI release after 0.1.8)** a secret-backed header shows the secret it reads instead: `x-api-key ← secret STRIPE_KEY`; on 0.1.3–0.1.8 it lists only the header name, indistinguishable from a literal (gated: `cron.read`) |
| `insta --agent cron create <name> <expression>` `--url <https://…>` \| `--service <name>` [`--path </api/cron>`] [`--method GET\|POST`] [`--body <json>`] [`--header k=v`]… [`--secret-ref k=SECRET`]… **(`--secret-ref`: the CLI release after 0.1.8; check `insta cron create --help`)** [`--timeout <ms>`] [`--branch <b>`] [`--json`] | schedule an HTTP call. Five-field cron, **UTC**, one-minute floor. `--service` targets a compute service in the **same org** — not merely the same project, since sharing one across an org's projects is legitimate. The CLI, though, resolves `--service <name>` only among the **current branch's** services and has no id form, so from the command line the target is always a service on this branch; a cross-project target is legal on the wire but has to be created through the API. `--url` any external https endpoint. `--method` defaults to GET, or POST when `--body` is given; `--timeout` is `1000`–`300000` ms. **Credentials go in `--secret-ref`, not `--header`** — see [Cron](#cron). Per-org quota by plan: **free 5, pro 100, team/enterprise 1000** — a full org answers 403 (gated: `cron.write`, **approve** by default) |
| `insta --agent cron edit <name>` [same flags as create, all optional] [`--json`] | change a schedule — `--name <new>` renames it. **The request is REPLACED, not merged.** Literal header values are write-only and cannot be read back, so `--header X=y` alone **drops every other literal header**, drops a POST body, and flips the method back to GET unless you restate them; the CLI warns on stderr and then proceeds. Secret refs are readable, so the CLI **carries them forward** unless `--header`/`--secret-ref` names that header again; `--secret-ref X=` (empty secret) removes one. Restate the whole request, or leave the request flags off entirely to change only the name/expression/timeout. Sends `If-Match` on the revision it just read — a 409 means someone else changed it meanwhile, and the CLI says so rather than re-reading and clobbering (gated: `cron.write`, **approve** by default) |
...
**Credentials: `--secret-ref <header>=<SECRET>`, not `--header`** (the CLI release after 0.1.8; `insta cron create --help` lists it once present). The job stores the secret's
**name**; the value is resolved at send time, on every attempt, from the job's project and branch — for
a `--service` target with the same precedence as that service's env (branch-scoped over project-scoped),
for a `--url` target from secrets not bound to a service. So a rotated secret (`insta --agent secrets
set`) reaches the next run with no edit to the job. Only secrets you set resolve — platform-minted
...
declaring the whole request. Secret refs are the exception the CLI can carry forward.
...
available. Via MCP: the `insta_send_feedback` tool takes the same fields (plus explicit
`projectId`/`branch`) and returns the same `ticket`; `insta_get_feedback_status` (`ticketId`) gives
its status.
