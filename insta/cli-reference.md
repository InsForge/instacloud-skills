...
registrar account and owns the zone, so nobody is asked for a registrant contact: after payment it
publishes the records itself and attaches the apex plus `www` with nothing for you to set. `buy`
answers a Stripe Checkout URL — **a human must pay**; an agent's job is to relay that URL and then
poll `insta --agent domain status <name>`.
