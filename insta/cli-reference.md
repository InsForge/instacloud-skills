...
| `insta --agent build-logs <build-id>` [`--source <archive\|github>`] [`--follow`] [`--json`] | **Requires `build-logs` in `insta --agent --help` and the deployed Platform build-log API.** Source-build steps/output via the Platform API. Default source is `archive`: use `insta --agent build-logs <id>` with the ID from the concrete command printed when `insta --agent deploy` accepts the operation (stderr with `deploy --json`). For `--source github`, use `build.buildId` from `insta --agent compute connect-repo <owner/repo> --json`, or `builds[].id` from `GET /projects/{id}/github/builds`. `--follow` polls while building and briefly after completion for delayed output. `--json` prints one snapshot and cannot be combined with `--follow`. Governed by `logs.read`. No Depot credentials are needed locally |
...
pre-1.0 but `insta --agent autoupdate off` and `INSTA_NO_AUTOUPDATE=1` turn it off, so an agent can be
...
sources`, and `secrets bindings` are all **names only**.
...
```sh
insta --agent services add compute web --volume 1 --mount-path /app/storage
