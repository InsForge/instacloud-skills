...
| `insta --agent domain attach <hostname>` [`--branch --group`] [`--json`] | **(CLI ≥ 0.0.76)** point a bought name, or **any subdomain of one**, at a compute service — the platform publishes the DNS in the zone it controls, so there are no records for you to set. `abc.com` binds `abc.com` **and** `www.abc.com`; `api.abc.com` binds only that, and every other hostname of the domain is left exactly as it is. So `api.abc.com` → one service and `docs.abc.com` → another is two calls, and both keep serving. A hostname that already serves **another** service is not moved by this: the compute plane still holds it, so the attach lands `failed` — release it there first (`insta --agent compute remove-domain <hostname> --group <old service>`), then attach. Two 409s: asking for what already serves that same service, and a domain whose previous DNS records are still being swept after a teardown — that one says `retry in a moment`, and means it. Also how a `detached` domain — its compute service was deleted, the registration stands — is bound again. Gated: `deploy` |
