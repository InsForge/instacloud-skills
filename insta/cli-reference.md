...
| `insta --agent cron delete <name>` [`--yes`] [`--json`] | delete the schedule — it stops firing immediately and **cannot be restored**, its run history is **no longer readable** (`cron runs` answers not found), and the **name is released** for reuse. `--yes` is required without a terminal, so an agent must always pass it (gated: `cron.delete`, **approve** by default) |
